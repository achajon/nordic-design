...
- Preservar 100% la identidad real: mismo rostro, expresión, sonrisa, rasgos — nada de reshapear o "embellecer" facciones. **Incluye explícitamente mantener el ángulo de cabeza y la dirección de la mirada idénticos a la foto de referencia** (rostro frontal mirando a cámara si así estaba en la foto original, mismo tilt de cabeza) — sin esa instrucción explícita, Gemini tiende a rotar la cabeza a un semi-perfil "más editorial", lo cual cambia la expresión percibida de la persona (pasó con Renato). Solo el cuerpo/hombros pueden angularse un poco, la cara no.
- Encuadre de medio cuerpo, alejado — NO un close-up directo a la cara. La cara debe ocupar una porción menor del cuadro (a lo mucho un tercio-mitad de la altura), con espacio alrededor.
- **Formato de salida vertical: decláralo explícito al inicio del prompt** ("output MUST be vertical/portrait, ~4:5, NOT landscape"), sobre todo cuando uses una foto de ambiente landscape como segunda referencia (ver el punto de fondo abajo) — el modelo puede dejarse influenciar por el aspect ratio de esa referencia y devolver una imagen horizontal (pasó en el primer intento con Renato). Si sale horizontal, es un fallo de formato, no una variante válida — vuelve a intentar.
...
- **Fondo: por default usa una foto real de ambiente del local** (ej. `nordic-images/reception-logo-wall-counter-02.jpeg`, el muro con el logo Nordic en madera) pasada como segunda `--image` de referencia junto a la(s) foto(s) de la persona, en vez de pedir un fondo genérico inventado. Pide explícitamente que el fondo quede **recortado/simplificado solo a los elementos limpios** (el muro y el logo) — nada de mostrador, productos, banca ni otro mobiliario/clutter de la foto de referencia, aunque aparezcan en ella. Desenfocado detrás de la persona (profundidad de campo).
  - **Excepción: si la foto original de la persona ya fue tomada dentro del local real** (pasó con Henry, cuya selfie ya tiene las luces LED hexagonales, sillas y espejos del salón de fondo) — no la reemplaces por el muro de recepción. Mantén su ambiente real tal cual, y solo pide limpiar/desenfocar lo que distraiga (ej. una TV con contenido legible) en vez de recortarlo a un solo elemento. Da variedad visual a la serie y es igual de "real" que usar la foto de la recepción — confirma con el usuario si no es obvio cuál conviene más para esa foto en particular.
...
Revisa el resultado abriéndolo en Preview antes de continuar. Si el encuadre sigue muy cerrado/directo, salió en horizontal en vez de vertical, la cabeza quedó rotada/angulada cambiando la expresión respecto a la foto original, el vestuario/props no corresponden al rol, el fondo trae de vuelta mobiliario/clutter, el fondo o la piel tienen tono de color en vez de blanco y negro puro, el dorado aparece sobre la cara en vez de en las herramientas, o la luz se ve plana sin sombras — vuelve a intentar con el prompt ajustado. No sigas con una versión que no cumpla estos puntos. **Muéstrale siempre la imagen retocada al usuario y espera su aprobación antes de seguir con el armado de los tiles** — no continues automáticamente solo porque el resultado te parece correcto a ti.
